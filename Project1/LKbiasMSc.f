...
C     ---   Commons   ---
      ...
      write(*,*) 'n=3 --> constant spatial density of stars' 
      write(*,*) 'n=2 --> spatial density of stars decreasing as R^(-1)' 
      write(*,*) 'n=1 --> spatial density of stars decreasing as R^(-2)' 
      read(*,*) exp    
...
c
      ...
      zmv=(zmvnp-(dsqrt(3.d0)*sigmam))+ran(seed)*range ! 'real' absolute magnitude with uniform distribution
      end if


       put=zminc*ran(seed)+zmmin
       dist=(put/g)**g
...
C     ---   pick two uniform numbers in the square (-1,+1)
	 ...
	 v1=2.0d0*ran(seed)-1.0d0
	 v2=2.0d0*ran(seed)-1.0d0
