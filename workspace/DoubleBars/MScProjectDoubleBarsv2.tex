...
\newpage
\section{Methods}
The model was created using OdeInt from Numerical Recipes \citep{press}. I first converted OdeInt to Fortran 90 for this purpose.
...
\end{equation}
These equations of motion are second-order ordinary differential equations (ODE) but any second-oder ODE can be written as a system
of two coupled first-order ODEs. As there are four bodies in the system and and x and y components for both of these we have a total of eight second-order 
ODEs which then gives us sixteen first-order ODEs to solve.
...
\begin{table}[ht!]
  ...
  \end{tabular}
\end{table}
Discuss mass of inner bar ration of outer bar
